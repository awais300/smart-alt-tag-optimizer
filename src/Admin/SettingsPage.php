...
class SettingsPage
{
	...
	public function register_settings()
	{
		...
		$this->register_setting('smartalt_ai_batch_prompt_template', '');
			...
			<div class="smartalt-field">
				...
				<select id="smartalt_alt_source" name="smartalt_alt_source">
					<option value="post_title" <?php selected($alt_source, 'post_title'); ?>>
						<?php esc_html_e('Post Title (Fast, No API)', SMARTALT_TEXT_DOMAIN); ?>
					</option>
					<option value="ai" <?php selected($alt_source, 'ai'); ?>>
						<?php esc_html_e('AI Generated (Requires API, 1 call per page)', SMARTALT_TEXT_DOMAIN); ?>
					</option>
				</select>
				<div class="smartalt-help">
					<?php esc_html_e('Choose the source for generating alt text. Post Title is instant. AI provides varied, contextual alts but requires an API key.', SMARTALT_TEXT_DOMAIN); ?>
				</div>
			</div>
	...
	?>
		<form method="post" action="options.php">
			...
			<div class="smartalt-field">
				...
				<div class="smartalt-help">
					<?php esc_html_e('Full HTTP(S) URL to your AI service endpoint. Only ONE API call is made per page load (batch processing).', SMARTALT_TEXT_DOMAIN); ?>
				</div>
			</div>
			...
			<div class="smartalt-field">
				<label for="smartalt_ai_request_template"><?php esc_html_e('Request Body Template (Single Image)', SMARTALT_TEXT_DOMAIN); ?></label>
				...
				<div class="smartalt-help">
					<?php esc_html_e('Used for bulk/attachment updates. Placeholders: {image_url}, {post_title}, {post_excerpt}, {post_content}, {image_filename}, {max_length}', SMARTALT_TEXT_DOMAIN); ?>
				</div>
			</div>

			<div class="smartalt-field">
				<label for="smartalt_ai_batch_prompt_template"><?php esc_html_e('Batch Prompt Template (Frontend)', SMARTALT_TEXT_DOMAIN); ?></label>
				<textarea id="smartalt_ai_batch_prompt_template" name="smartalt_ai_batch_prompt_template" placeholder=""><?php echo esc_textarea($batch_prompt_template); ?></textarea>
				<div class="smartalt-help">
					<?php esc_html_e('Used for on-the-fly batch processing (ONE API call for all images on a page). Leave empty to use default. Placeholders: {image_count}, {post_title}, {post_excerpt}, {post_content}, {images_json}, {max_length}. {images_json} contains URL, filename, and position info.', SMARTALT_TEXT_DOMAIN); ?>
				</div>
				<button type="button" class="button button-small" id="smartalt-reset-batch-template">
					<?php esc_html_e('Reset to Default', SMARTALT_TEXT_DOMAIN); ?>
				</button>
			</div>
			...
			<div class="smartalt-field">
				...
				<div class="smartalt-help">
					<?php esc_html_e('Dot notation path to alt texts in JSON response (e.g., "choices[0].message.content" for OpenAI, or "data.alts" for custom).', SMARTALT_TEXT_DOMAIN); ?>
				</div>
			</div>
		...
		<script>
			document.getElementById('smartalt-test-connection')?.addEventListener('click', function() {
				if (confirm('<?php esc_attr_e('Test AI connection? This will make a test API call.', SMARTALT_TEXT_DOMAIN); ?>')) {
					fetch(ajaxurl, {
						method: 'POST',
						headers: {
							'Content-Type': 'application/x-www-form-urlencoded'
						},
						body: 'action=smartalt_test_connection&_wpnonce=' + document.querySelector('input[name="_wpnonce"]').value
			...
			document.getElementById('smartalt-clear-ai-cache')?.addEventListener('click', function() {
				if (confirm('<?php esc_attr_e('Clear all cached AI results? This cannot be undone.', SMARTALT_TEXT_DOMAIN); ?>')) {
					fetch(ajaxurl, {
						...
						body: 'action=smartalt_clear_ai_cache&_wpnonce=' + document.querySelector('input[name="_wpnonce"]').value
					}).then(r => r.json()).then(d => {
			...
			document.getElementById('smartalt-reset-batch-template')?.addEventListener('click', function(e) {
				...
				alert('<?php esc_attr_e('Default template loaded!', SMARTALT_TEXT_DOMAIN); ?>');
			});
		</script>
	<?php
	}
